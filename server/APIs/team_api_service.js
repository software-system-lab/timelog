  ...
  SetAPI () {
    this.router.get('/name', async (req, res) => {
      try {
        ...
        const result = await _teamProvider.getTeamName(teamID)
    ...
    this.router.post('/create', async (req, res) => {
      try {
        const result = await _teamProvider.createTeam(req.body)
        if (result) {
          res.send({
            TeamID: result
          })
    ...
    this.router.post('/join', async (req, res) => {
      try {
        const result = await _teamProvider.joinTeam(req.body)
        switch (result) {
          case 'Not found':
            res.status(404)
            res.send(result)

...
module.exports = class {
  getTeamName (teamID) {
    ...
    return DB.query(cmd, [teamID])
  }

  async createTeam (params) {
    var createTeamCmd = 'INSERT INTO `team` (`TeamName`, `TeamAccessCode`, `Joinable`, `CreatorID`) VALUES (?, ?, ?, ?)'
    const teamResult = await DB.query(createTeamCmd, [params.TeamName, params.TeamCode, true, params.UserID])
    var createUserTeamMapCmd = 'INSERT INTO `user_team_mapping` (`UserID`, `TeamID`) VALUES(?, ?)'
    const mapResult = await (DB.query(createUserTeamMapCmd, [params.UserID, teamResult.insertId]))
    if (mapResult) {
  ...
  async joinTeam (params) {
    ...
    const result = await DB.query(cmd, [params.UserID, teamID])
    if (result) {
      return teamID
    }

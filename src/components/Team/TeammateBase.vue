  ...
  export default {
    ...
    methods: {
      async QueryTeammateData() {
        let result = await _logService.TagsAndLengthOfTime(this.teammate.FBUserID);
        if (result != "no data") {
          ...
          for (let i = 0; i < result.length; i++) {
            if (i < 5) {
              ...
              this.Data.datasets[1].data.push(result[i].TimeTarget != null ? result[i].TimeTarget.toFixed(0) * 60 : 0);
            } else if (i == 5) {
              ...
              this.Data.datasets[1].data[5] += result[i].TimeTarget != null ? result[i].TimeTarget.toFixed(0) * 60 : 0;
            }
            this.Data.datasets[0].TimeLengthSum += result[i].TimeLength;
            this.Data.datasets[1].TargetTimeLengthSum += result[i].TimeTarget * 60;
          }
        }
      },
      paddingLeft(str, len) {
